...
module Http2k
  module Helper
    ...
    def nginx_certs(app, cdomain)
      ...
      nginx_cert_item = begin
        data_bag_item('certs', app)
    ...
    def get_organization_uuid
      ...
      Chef::Role.list.each_key do |m_key|
        ...
        if !m.override_attributes['redborder'].nil? && !m.override_attributes['redborder']['organization_uuid'].nil? && !m.override_attributes['redborder']['sensor_uuid'].nil? && (m.override_attributes['redborder']['sensor_uuid'] == m.override_attributes['redborder']['organization_uuid'])
          organizations << m
        end
      end
      organizations
